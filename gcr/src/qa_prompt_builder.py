        ...
        if "paths" in question_dict:
            ...
            if self.add_rule:
                ...
                if len(rules) > 0:
                    ...
                    paths_list = utils.dfs(
                        g, question_dict["q_entity"], self.index_path_length
                    )
            else:
                paths_list = utils.dfs(
                    g, question_dict["q_entity"], self.index_path_length
...
"""
    ...
    def get_graph_index(self, question_dict):
        ...
        paths_list_str = [
            f"{self.PATH_START_TOKEN}{utils.path_to_string(path)}{self.PATH_END_TOKEN}"
...
Please generate triples that are relevant to the question."""
    ...
    def get_graph_index(self, question_dict):
        ...
        tokenized_entity_list = self.tokenizer(
            list(entity_list), padding=False, add_special_tokens=False
        ).input_ids
        ...
        tokenized_relation_list = self.tokenizer(
            list(relation_list), padding=False, add_special_tokens=False
        ).input_ids
        ...
        tokenized_triple_list = self.tokenizer(
            triple_list_str, padding=False, add_special_tokens=False
        ).input_ids
        ...
        return (
            [entity_trie, relation_trie, triple_trie],
            entity_list,
            relation_list,
            triple_list,
            paths_list,
    ...
    def process_input(self, question_dict, return_tire=True):
        ...
        trie = None
        if return_tire:
            trie, entity_list, relation_list, triple_list, paths_list = (
                self.get_graph_index(question_dict)
            )
        ...
        return (
            [entity_query, relation_query, triple_query],
            ground_paths,
            trie,
            entity_list,
            relation_list,
