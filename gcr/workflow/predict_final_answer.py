...
from gcr.src.utils.qa_utils import eval_result
import json
from multiprocessing.dummy import Pool
from functools import partial
from gcr.src.qa_prompt_builder import PromptBuilder
from gcr.src.utils.graph_utils import build_graph, get_truth_paths
from gcr.src.utils import path_to_string, load_jsonl, dfs
...
def make_prediction(data, args, processed_list, input_builder, model):
    ...
    if "gcr" in args.model_name:
        ...
        tokenized_entity_list = model.tokenizer(
    ...
    if args.add_path:
        ...
        if "use_assistant_model" in vars(args) and args.use_assistant_model:
            ...
            if len(assistant_model_name) > 64:
                assistant_model_name_md5 = hashlib.md5(
                    assistant_model_name.encode()
                ).hexdigest()
                assistant_model_name = (
                    assistant_model_name[:64] + "_" + assistant_model_name_md5
                )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--add_rule", type=lambda x: (str(x).lower() == "true"), default=False
    )
    argparser.add_argument(
