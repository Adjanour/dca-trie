...
def index_graph(args):
    ...
    with Pool(args.n) as p:
        for res in tqdm.tqdm(
            p.imap_unordered(partial(process, undirected=args.undirected), dataset),
            total=len(dataset),
        ):
            results.append(res)
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--output_path", type=str, default="data/shortest_path_index"
    )
    argparser.add_argument(
        "--undirected", action="store_true", help="whether the graph is undirected"
    )
